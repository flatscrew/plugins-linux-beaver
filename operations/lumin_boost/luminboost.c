/* GEGL operation implementing Krita's Increase Luminosity blend mode
 *
...
#define GEGL_OP_NAME     increase_luminosity
#define GEGL_OP_C_SOURCE luminboost.c
  ...
  for (i = 0; i < n_pixels; i++)
    {
      ...
      /* Approximate luma (sRGB weights) */
      gfloat luma_b = 0.299f * b_r + 0.587f * b_g + 0.114f * b_b;
      gfloat luma_a = 0.299f * a_r + 0.587f * a_g + 0.114f * a_b;

      /* Increase luminosity if aux luma is higher, use max component */
      gfloat processed_r = b_r;
      gfloat processed_g = b_g;
      gfloat processed_b = b_b;
      if (luma_a > luma_b)
        {
          processed_r = MAX(b_r, a_r);
          processed_g = MAX(b_g, a_g);
          processed_b = MAX(b_b, a_b);
        }
...
gegl_op_class_init (GeglOpClass *klass)
{
  ...
  gegl_operation_class_set_keys (operation_class,
    "name",        "ai/lb:increase-luminosity",
    "title",       _("Increase Luminosity Blend Mode"),
    "reference-hash", "increaseluminosityfairygegl",
    "categories", "hidden",
    "description", _("Increase Luminosity blend mode, increasing the luminosity of the input layer if the aux layer's luminosity is higher"),
    NULL);
}
